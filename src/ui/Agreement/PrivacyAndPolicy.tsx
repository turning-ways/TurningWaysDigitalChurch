...
const PrivacyAndPolicy = ({ onClose }: { onClose: () => void }) => {
	...
	return (
		<Modal onClose={onClose} className="">
			<main className="bg-white max-w-[1000px] w-full max-h-[550px] overflow-y-scroll">
				...
				<section className="md:grid md:grid-cols-[350px,1fr] gap-x-4 text-[#1C1C1C] p-4">
					<ul className="space-y-2 hidden md:block sticky top-20 h-fit">
						{termList.map((item) => (
							<li className={`${active === item.name && "font-azoBold"}`}>
								<a href={`#${item.name}`} onClick={() => setActive(item.name)}>
									{item.name}
								</a>
							</li>
						))}
					...
					<ul className="space-y-10">
						<li className="space-y-4">
							<h1 className="font-azoBold underline">{Agreement.title.toUpperCase()}</h1>
							<p>{Agreement.p1}</p>
							<p>{Agreement.p2}</p>
							<p>
								{Agreement.p3} <span className="font-azoBold">{Agreement.p3Bold}</span>
							</p>
							<p>{Agreement.p4}</p>
							<p>{Agreement.p5}</p>
						</li>
						<li className="space-y-4">
							<h1 className="font-azoBold underline">{OurServices.title.toUpperCase()}</h1>
							<p>{OurServices.p1}</p>
						</li>
						<li className="space-y-4">
							<h1 className="font-azoBold underline" id={PropertyRights.title.toUpperCase()}>
						...
						<li>
							<h1 className="font-azoBold underline">{UserRepresentations.title}</h1>
							<p>{UserRepresentations.p1}</p>
							<p>{UserRepresentations.p2}</p>
						</li>
						<li>
							<h1 className="font-azoBold underline">{ProhibitedActivites.title}</h1>
