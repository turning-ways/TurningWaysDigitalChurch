...
export const fetchContacts = createAsyncThunk(
	...
	async (churchId: string, { signal, rejectWithValue }) => {
		try {
			const response = await axios.get(`/api/v1/contacts/${churchId}`, { signal });
...
export const fetchContact = createAsyncThunk(
	...
	async (
		...
		try {
			const response = await axios.get(`/api/v1/contacts/${churchId}/contact/${contactId}`, {
				signal,
			});
...
export const createContact = createAsyncThunk(
	...
	async (
		...
		try {
			const response = await axios.post(`/api/v1/contacts/${churchId}`, contact, { signal });
			onClose();
			return response.data.data.contact;
		} catch (error) {
...
export const updateContactStatus = createAsyncThunk(
	...
	async (
		{ churchId, contactId, status }: { churchId: string; contactId: string; status: string },
		{ signal, rejectWithValue }
	) => {
		try {
...
export const removeLabelFromContact = createAsyncThunk(
	...
	async (
		{ churchId, contactId, labelId }: { churchId: string; contactId: string; labelId: string },
		{ signal, rejectWithValue }
	) => {
		try {
			await axios.delete(`/api/v1/contacts/${churchId}/contact/${contactId}/label/${labelId}`, {
				signal,
			});
			return labelId;
		} catch (error: unknown) {
...
export const updateActionToContact = createAsyncThunk(
	...
	async (
		{ churchId, contactId, actionId }: { churchId: string; contactId: string; actionId: string },
		{ signal, rejectWithValue }
	) => {
		try {
			const response = await axios.patch(
...
export const removeActionFromContact = createAsyncThunk(
	...
	async (
		...
		try {
			await axios.delete(`/api/v1/contacts/${churchId}/contact/${contactId}/action/${actionId}`, {
				signal,
			});
			return actionId;
		} catch (error: unknown) {
...
export const assignMemberToContact = createAsyncThunk(
	...
	async (
		{ churchId, contactId, memberId }: { churchId: string; contactId: string; memberId: string },
		{ signal, rejectWithValue }
	) => {
		try {
			const response = await axios.post(
...
export const addCommentToContact = createAsyncThunk(
	...
	async (
		{ churchId, contactId, note }: { churchId: string; contactId: string; note: string },
		{ signal, rejectWithValue }
	) => {
		const user: Authuser = JSON.parse(localStorage.getItem("user") as string);
		try {
...
export const removeCommentFromContact = createAsyncThunk(
	...
	async (
		{ churchId, contactId, noteId }: { churchId: string; contactId: string; noteId: string },
		{ signal, rejectWithValue }
	) => {
		try {
			await axios.delete(`/api/v1/contacts/${churchId}/contact/${contactId}/notes/${noteId}`, {
				signal,
			});
			return noteId;
		} catch (error: unknown) {
	...
	extraReducers: (builder) => {
		builder
			...
			.addCase(updateCommentToContact.fulfilled, (state, action) => {
				state.selectedContact!.notes = state.selectedContact!.notes.map((note) =>
...
export const selectStatusLoading = (state: RootState) => state.contacts.statusLoading;
export const selectLabelLoading = (state: RootState) => state.contacts.labelLoading;
export const selectAssignedLoading = (state: RootState) => state.contacts.assignedLoading;
export const selectNoteLoading = (state: RootState) => state.contacts.noteLoading;
