...
export const fetchMemberDetails = createAsyncThunk(
	...
	async ({ churchId, memberId }: { churchId: string; memberId: string }, { signal }) => {
		const response = await axios.get(`/api/v1/members/${churchId}/member/${memberId}`, { signal });
...
export const getAllMembers = createAsyncThunk(
	...
	async ({ churchId }: { churchId: string }, { signal }) => {
		const response = await axios.get(`/api/v1/churches/${churchId}/members`, { signal });
...
const memberSlice = createSlice({
	...
	reducers: {
		...
		updateTempMemberField(
			state,
			action: PayloadAction<{ field: string | string; value: string | number | boolean }>
		) {
...
export const selectMembers = (state: RootState): Member[] => state.members.members;
...
export const { clearMemberDetails, setMemberDetails, setTempMemberDetails, updateTempMemberField } =
	memberSlice.actions;
