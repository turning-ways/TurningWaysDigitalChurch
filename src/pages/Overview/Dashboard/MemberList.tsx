...
const MemberList: React.FC<TimeLine> = ({ timeLine }) => {
	const { data } = useMemberStats(timeLine);
	const members = data?.MembersJoined.members;
	const items = [{ name: "All" }, { name: "First Timers" }, { name: "Upcoming Birthday" }];
	...
	// 	(member: { anniversary: string }) => isAnniversary(member.anniversary) === true
	// );
					...
					{items.map((item: { name: string }, index: number) => (
						<li
							className={`p-3 ${
								...
								(item.name === "Upcoming Birthday" || item.name === "First Timers") &&
								"whitespace-nowrap"
							}`}
							onClick={() => {
								setActive(item.name);
        ...
        >
          Export Data
			...
			<div>
				...
				{active === "First Timers" && firstTimers && firstTimers.length === 0 && (
				...
				{active === "Upcoming Birthday" &&
					...
					birthdayCelebrants.length === 0 && (
						<p className="text-[#CACACA] mt-2">No upcoming birthday's at the moment</p>
					)}
				...
				{active === "Upcoming Anniversary" &&
					...
					upcomingAnniversary.length === 0 && (
						<p className="text-[#CACACA] mt-2">No upcoming anniversary at the moment</p>
					)} */}
			</div>
		</>
	);
