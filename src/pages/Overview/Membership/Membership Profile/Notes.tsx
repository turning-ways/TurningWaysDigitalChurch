...
const Notes: React.FC<NotesProps> = ({ openNote, onClose }) => {
	...
	const { mutate: del, isPending: pendingDelete } = useDeleteNote(memberId ?? "");
	...
	const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
		if (e.key === "Enter") {
			mutate({ note: value, createdBy: createdBy });
			refetch();
		}
	};
	...
	return (
		<>
			<div className={` bg-slate-600 justify-center  ${openNote ? "flex" : "hidden"}`}>
				<div className="absolute bottom-0 right-10 mb-2 flex justify-between  h-[700px] overflow-y-scroll text-white flex-col border bg-white px-2">
					<div className="bg-white p-4 md:w-[600px] w-[80svw]">
						<div className="flex justify-between">
							...
							<IoClose className="text-[#141414] text-2xl cursor-pointer" onClick={onClose} />
						</div>
						{notes &&
							notes.map((note: { comment: string; date: string; id: string }, index: number) => (
								<div className="flex space-x-3 mt-4">
									<div className="bg-[#D9D9D9] text-[#2A2A2A] h-10 w-10 flex items-center justify-center rounded-full">
										{data?.member?.profile?.firstName?.charAt(0)?.toUpperCase()}
									</div>
									<div className="space-y-2 w-full">
										<div className="flex justify-between text-[#727171] ">
											<p className="">{data?.member?.profile?.firstName}</p>
											<p>{formatDate(note.date) + " " + getTime(note.date)}</p>
										</div>
										<input
											...
											onChange={(e) => handleEditComment(note.id, e.target.value)}
