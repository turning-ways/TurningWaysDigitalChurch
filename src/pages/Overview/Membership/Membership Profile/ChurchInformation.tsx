	...
	const information = [
		{
			...
			value:
				member && member?.memberStatus
					? member?.memberStatus?.slice(0, 1).toUpperCase() + member?.memberStatus?.slice(1)
					: "Undefined",
		},
		{
			name: "Worker Type",
			value:
				member && member?.profile?.worker
					? member?.profile?.worker?.slice(0, 1).toUpperCase() + member?.profile?.worker?.slice(1)
					: "Undefined",
		},
		{
		...
		<div className="">
			...
			<div className="flex justify-between">
				<button
					...
					onClick={() => navigate(`/admin/directory/member/contact-information?id=${memberId}`)}>
					<p className="text-lg ">Previous</p>
				</button>
				<button
					className=" flex mt-4 bg-[#17275B] text-white px-4 py-2  rounded-lg gap-2 justify-center "
					onClick={() => navigate(`/admin/directory/member/membership-history?id=${memberId}`)}>
