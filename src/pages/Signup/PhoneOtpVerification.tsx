...
import { useVerifyPhoneAuthOtp } from "../../hooks/useAuthData";
...
const PhoneOtpVerification = () => {
	...
	const { mutate, isPending } = useVerifyPhoneAuthOtp();
	const { mutate: sendOtp } = useRegisterWithPhone(recaptchaContainerRef);
	return (
		<>
				...
				<p className="mt-3">
					...
					<button
						className={`text-[#CCE9D1]  ${
							timerActive ? "cursor-not-allowed" : "cursor-pointer hover:text-[#61BD74]"
						}`}
						...
						onClick={() => {
							...
							sendOtp({
								phoneNumber: phone,
								firstName: window.userDetails.firstName,
								lastName: window.userDetails.lastName,
								password: window.userDetails.password,
								passwordConfirm: window.userDetails.passwordConfirm,
							});
						}}>
						Resend Code
					</button>
